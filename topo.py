...
def create_complex_topology_with_nat():
    net = Mininet(
        controller=None,
    ...
    # ====================================================================
    # === NUOVO ROUTER NAT E SERVER ======================================
    # ====================================================================
    info('*** Aggiunta del Router NAT e del Server\n')
    # nat_router è uno switch OpenFlow che il controller gestirà
    nat_router = net.addSwitch('nat_router', dpid='0000000000000006')
    
    # Il server si trova in una rete privata "dietro" il NAT
    server = net.addHost(
        'server', 
        ip='192.168.100.10/24', 
        mac='AA:BB:CC:DD:EE:FF', 
        defaultRoute="via 192.168.100.1" # Il gateway è l'IP interno del NAT
    )
    # ====================================================================

    info('*** Creazione dei collegamenti\n')
    ...
    # ====================================================================
    # === NUOVI COLLEGAMENTI PER IL NAT ==================================
    # ====================================================================
    # Colleghiamo la rete "pubblica" (rappresentata da r2) al nat_router
    # r2:porta3 <--> nat_router:porta1 (lato pubblico)
    net.addLink(r2, nat_router, port1=3, port2=1)
    
    # Colleghiamo il server al nat_router
    # nat_router:porta2 <--> server (lato privato)
...
if __name__ == '__main__':
    setLogLevel('info')
    create_complex_topology_with_nat()